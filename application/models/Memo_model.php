...
class Memo_model extends CI_Model {
	...
	public function __construct()
	{
		...
		$this->load->helper(array('file', 'path'));
	...
	public function get_contents($owner_id, $path_segments, $sort, $sort_asc)
	{
		...
		if(!empty($contents)){
			...
			// Memasukkan data contents yang bertipe file ke array files
			foreach($contents as $content){
				...
				if(is_dir($directory . '/' . $content['name'])){
					$content['url'] = site_url('memo/folder' . $path . '/' . $this->_encode($content['name']));
					$content['type'] = 'Folder';
					array_push($folders, $content);
				}
				else if(is_file($directory . '/' . $content['name'])){
					$content['url'] = site_url('memo/file' . $path . '/' . $this->_encode($content['name']));
